...
public class RongCloudBean {
	...
	public void destoryChatRoom(String roomId) throws Exception {
		CodeSuccessReslut result = rongCloud.chatroom.destroy(new String[] { roomId });
		if (result.getCode() != 200) {
			throw new Exception(result.getCode() + ":" + result.getErrorMessage());
		}
	}
	...
	public void sendMessageToChatRoom(String userId, long roomId, BaseMessage msg) throws Exception {
		CodeSuccessReslut result = rongCloud.message.publishChatroom(userId, new String[] { roomId + "" }, msg);
		if (result.getCode() != 200) {
	...
	public void sendSystemMessage(List<Long> memberIds, BaseMessage message) throws Exception {
		...
		CodeSuccessReslut result= this.rongCloud.message.PublishSystem(Keys.RONGCLOUD_SYSTEM_ID, arr, message, null, null, 1, 1);
		if (result.getCode() != 200) {
			throw new Exception("" + result.getCode() + ":" + result.getErrorMessage());
		}
	...
	public void sendSystemMessage(String[] memberIds, BaseMessage message) throws Exception {
		...
		CodeSuccessReslut result= this.rongCloud.message.PublishSystem(Keys.RONGCLOUD_SYSTEM_ID, memberIds, message, null, null, 1, 1);
		if (result.getCode() != 200) {
			throw new Exception("" + result.getCode() + ":" + result.getErrorMessage());
		}
	...
	public void sendGroup(String senderId, String groupId, BaseMessage message) throws Exception {
		CodeSuccessReslut result= this.rongCloud.message.publishGroup(senderId, new String[]{groupId}, message, null, null, 1, 1, 1);
		if (result.getCode() != 200) {
			throw new Exception("" + result.getCode() + ":" + result.getErrorMessage());
		}
	}
