...
public class PushExample {
	...
	private static final String appKey = Keys.JPhsh.APP_KEY;
	private static final String masterSecret = Keys.JPhsh.MARKET_SECRET;

	protected static final String APP_KEY = Keys.JPhsh.APP_KEY;
	protected static final String MASTER_SECRET = Keys.JPhsh.MARKET_SECRET;
	...
	public static void testSendPushesWithMultiCallback() {
		NettyHttpClient client = new NettyHttpClient(ServiceHelper.getBasicAuthorization(APP_KEY, MASTER_SECRET), null,
	...
	public static void testSendPush() {
		...
		PushPayload payload = PushPayload.newBuilder().setPlatform(Platform.all()).setAudience(Audience.alias(Keys.JPhsh.ALIAS))
				.setNotification(Notification.alert("极光推送测试")).build();
		try {
	...
	public static PushPayload buildPushObject_all_alias_alert() {
		return PushPayload.newBuilder().setPlatform(Platform.all()).setAudience(Audience.alias("test_1964205138"))
	...
	public static PushPayload buildPushObject_android_newly_support() {
		...
		Notification notification = Notification.newBuilder()
				.addPlatformNotification(
						AndroidNotification.newBuilder().setAlert(ALERT).setBigPicPath("path to big picture")
								.setBigText("long text").setBuilderId(1).setCategory("CATEGORY_SOCIAL").setInbox(inbox)
								.setStyle(1).setTitle("Alert test").setPriority(1).build())
				.build();
		return PushPayload.newBuilder().setPlatform(Platform.all())
				.setAudience(Audience.registrationId("18071adc030dcba91c0")).setNotification(notification)
				.setOptions(Options.sendno()).build();
	}
